...
app_ui = ui.page_sidebar(
    ...
    ui.page_navbar(
        ui.nav_panel(
            ...
            ui.layout_columns(
                output_widget("expression_altplot")
            )
        ),
        ui.nav_panel(
            "Gene Methylation",
...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    def filtered_expr() -> pd.DataFrame:
        data = pq.read_table('ALL_RPKM_LABELED_FILTERED.parquet', columns=["AGE", "SEX", "LINE", input.gene(), input.altGene()]).to_pandas()
